...
idxs_train = [x in files_train for x in merged.filename.to_list()]
# get the image ids and labels for training set
ids_train = merged.image_id[idxs_train].to_list()
fileid_train = [str(i) for i in ids_train] 
labels_train = merged.majority[idxs_train].to_list()

# create a boolean index for test set to filter the dataframe
idxs_test = [x in files_test for x in merged.filename.to_list()]
# get the image ids and labels for test set
ids_test = merged.image_id[idxs_test].to_list()
fileid_test = [str(i) for i in ids_test]
labels_test = merged.majority[idxs_test].to_list()

print('TRAINING OVERVIEW:\n')
print('==================\n')
print('Number of patients in train: ',len(files_train),'(%.2f percent)' % (100*len(files_train)/len(merged.filename.unique())))
print('Number of patients in test: ',len(files_test),'(%.2f percent)' % (100*len(files_test)/len(merged.filename.unique())))

print('\nNumber of NMF in dataset: ',merged.majority.value_counts().get('NMF', 0),'(%.2f percent)' % (100*merged.majority.value_counts().get('NMF', 0)/merged.majority.count()))
...
print('\nNumber of AMF in dataset: ',merged.majority.value_counts().get('AMF', 0),'(%.2f percent)' % (100*merged.majority.value_counts().get('AMF', 0)/merged.majority.count()))
